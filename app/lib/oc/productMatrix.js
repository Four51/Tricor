...
function productmatrix() {
    ...
    function template() {
        return [
            ...
            '<div class="col-xs-2" ng-show="product.DisplayInventory"><div>{{\'Qty Avail\' | r}}</div><div ng-repeat="item in group | orderobjectby:\'ListOrder\':false | filter:{Show: true}">{{item.QuantityAvailable}}</div></div>',
            ...
            '<div class="col-xs-2" ng-show="product.DisplayInventory"><div>{{\'Qty Avail\' | r}}</div><div ng-repeat="group in comboVariants | orderobjectby:\'ListOrder\':false | filter:{Show: true}">{{group.QuantityAvailable}}</div></div>',
